  ...
  return (
    <View style={props.style}>
      <TouchableOpacity onPress={() => setVisible(!visible)} style={[styles.select,{ width: props.width && props.width, alignSelf: "center" },]}>
        ...
        <Image source={images.dropdownarrow} style={styles.dropdown}/>
      </TouchableOpacity>
      <Modal 
        animationType={"slide"} 
        transparent={true} 
          ...
          <View style={{ flex: 1, backgroundColor: colors.white, borderTopRightRadius: 20, borderTopLeftRadius: 20 }}>
            ...
            <FlatList
                ...
                renderItem={({item,index})=>{
                    return  <TouchableOpacity onPress={()=>{setItem(item.name);setVisible(!visible);props.onPress(item)}} style={[styles.item,{backgroundColor: index%2 ? "white" : "#E6E6E6"}]}>
                    <Text>{item.name}</Text>
                    </TouchableOpacity>
                }}
            />
